...
public class MongoArimaRepository implements ArimaRepository {
    ...
    public Map<String, List<ArimaData>> obtenerSeriesTemporales(
        ...
        List<Document> pipeline = Arrays.asList(
                ...
                // 2. Normalizar valores (igual que Z-Score)
                new Document("$project", new Document()
                        ...
                        .append("valor", new Document("$cond", Arrays.asList(
                                // Normalizar a decimal si está en porcentaje
                ...
                // 3. Ordenar por período (importante para series temporales)
        ...
        // Agrupar por clave (puesto-indicador-concepto-sucursal-negocio)
    ...
    @Override
    public List<ArimaData> obtenerDatosActuales(
            String periodo, String sucursal, ArimaConfig config) {

        log.info("📋 Obteniendo datos actuales - Período: {}", periodo);

        Document filtro = new Document("periodoActual", periodo);

        // NO excluir conceptos en datos actuales
